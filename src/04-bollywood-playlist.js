...
export function buildPlaylist(songs, maxDuration) {
  ...
  let totalAddSongDur = 0;
  let i = 0;

  while (i < songs.length) {
    const currentSongDur = songs[i];

    if (isNaN(currentSongDur) || currentSongDur <= 0) {
      i++;
      continue;
    }

    if (totalAddSongDur + currentSongDur > maxDuration) {
      break;
    }

    totalAddSongDur += currentSongDur;
    addedSongCount++;

    i++;
  }

  return { count: addedSongCount, totalDuration: totalAddSongDur };
}
